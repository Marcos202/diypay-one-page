-- Fix customer data fallbacks and improve webhook duplicity prevention
...
BEGIN
  ...
  -- 2. PRIORITY SELECTION with stronger tie-breaking: Find the most specific webhook
  ...
  ORDER BY 
    ...
    we.created_at DESC  -- Most recent if multiple exist (FIXED: Added proper tie-breaking)
  LIMIT 1;
  ...
  -- 3. BUILD RICH PAYLOAD with REAL customer data (NO FALLBACKS)
  rich_payload := jsonb_build_object(
    ...
    'data', jsonb_build_object(
      ...
      'Customer', jsonb_build_object(
        'full_name', sale_info.customer_full_name,  -- REMOVED: Fallback to 'Cliente'
        'email', sale_info.buyer_email,
        'mobile', sale_info.customer_mobile        -- REMOVED: Fallback to '+5511999999999'
      ),
  ...
  -- 4. DUPLICATE PREVENTION: Check if a job already exists for this event
  IF EXISTS (
    SELECT 1 FROM public.webhook_delivery_jobs 
    WHERE transaction_event_id = NEW.id 
      AND webhook_endpoint_id = selected_webhook.id
  ) THEN
    RAISE WARNING '[enqueue_webhook_deliveries] Job already exists for event_id=%, webhook_id=%', NEW.id, selected_webhook.id;
    RETURN NEW;
  END IF;

  -- 5. Create ONLY ONE delivery job for the selected webhook
